...
const fetchStarterItems = async () => {
  ...
  const allItemData = [];
  do {
    const response = await fetch('https://wiki.smite2.com/api.php?action=query&cmtitle=Category:Starter_items&format=json&list=categorymembers&cmlimit=500' + (cmContinue ? `&cmcontinue=${cmContinue}` : ''));
...
export default fetchStarterItems;